...
function mostActiveUsers (res) {
  try {
    var file = './generated/appmaker-author-stats.json';
...
function topEvents (res) {
  try {
    var file = './generated/appmaker-top-events.json';
    mostActive = fs.readFileSync( file, 'utf-8' );
    mostActive = JSON.parse( mostActive );

    var output = geckoboard.funnel(mostActive, 'eventCount', 'eventName');
    res.json( output );
  }
  catch ( e ) {
    console.log(e);
...
function topBricks (res) {
  try {
    var file = './generated/appmaker-top-bricks.json';
    mostActive = fs.readFileSync( file, 'utf-8' );
    mostActive = JSON.parse( mostActive );

    var output = geckoboard.funnel(mostActive, 'usageCount', 'brickName');
    res.json( output );
  }
  catch ( e ) {
    console.log(e);
...
function topRemixes (res) {
  try {
    var file = './generated/appmaker-most-remixed.json';
    mostActive = fs.readFileSync( file, 'utf-8' );
    mostActive = JSON.parse( mostActive );

    var output = geckoboard.funnel(mostActive, 'remixClickCount', 'appName');
    res.json( output );
  }
  catch ( e ) {
    console.log(e);
