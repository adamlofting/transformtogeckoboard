...
function addOrInsertToBackfill (backfill, row, fieldName) {
  var valueToCheck = row[0];
  var countToAdd = parseInt(row[1]);
  var matched = false;
  for (var i = 0; i < backfill.length; i++) {
    if (backfill[i][fieldName] && backfill[i][fieldName] === valueToCheck) {
      //cast to int
...
function getEOYTransactionsByCountry (callback) {
  ...
  function (err, response) {
    ...
    for (var i = 0; i < response.rows.length; i++) {
      output = addOrInsertToBackfill(output, response.rows[i], "country");
    }
...
function getEOYTransactionsByLanguage (callback) {
  analytics.data.ga.get( {
    auth: oauth2Client,
    "ids": process.env.GA_VIEW_ID_EOY,
    "start-date": "2015-10-04",
  ...
  function (err, response) {
    ...
    var output = backfill.eoyLanguage2015OldForm;
    for (var i = 0; i < response.rows.length; i++) {
      output = addOrInsertToBackfill(output, response.rows[i], "language");
    }
    output.sort(compareDonations);

    fs.writeFileSync( './generated/eoy-transactions-by-language.json', JSON.stringify( output ) );
    callback();
  });
}
...
function getEOYTransactionsBySource (callback) {
  ...
  function (err, response) {
    ...
    // 2015 transactions on old send.mo GA profile we want to add to the totals
    // for the new GA view for donate.mozilla.org
    // [{"source":"Snippet","eoyDonations":"16495"},
    // {"source":"Email","eoyDonations":"3"},
    // {"source":"Mozilla.org","eoyDonations":"1121"},
    // {"source":"Organic","eoyDonations":"496"},
    // {"source":"Directory Tiles","eoyDonations":"0"},
    // {"source":"Twitter & Facebook","eoyDonations":"13"},
    // {"source":"Other Referral","eoyDonations":"1318"}]
